          ...
          <Card key={project.id} sx={{ cursor: 'pointer' }} onClick={() => navigate(`/DetailsProject/${project.id}`)}>
            <CardContent sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1, flexShrink: 0 }}>
                <Button variant="outlined" color="primary" startIcon={<EditIcon />} onClick={(e) => handleEditProject(e, project)} sx={{ whiteSpace: 'nowrap' }}>
                  ערוך
                </Button>
                <Button variant="outlined" color="error" startIcon={<DeleteIcon />} onClick={(e) => { e.stopPropagation(); dispatch(deleteProject(project.id)); }} sx={{ whiteSpace: 'nowrap' }}>
                  מחק
                </Button>
              </Box>
            </CardContent>
